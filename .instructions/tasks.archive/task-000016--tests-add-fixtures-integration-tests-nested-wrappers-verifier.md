...
- 2026-01-14: Added a new nested wrapper fixture with explicit `<Envelope>` / `<Header>` / `<Payload>` / `<Footer>` structure.
- 2026-01-14: Added integration-style tests that split the fixture via `TxtMarkupSplitterService`, then validate outputs using `SplitOutputVerifier`.
- 2026-01-14: Added a negative test that corrupts one output chunk and asserts verifier failure (mismatch diagnostics).
- 2026-01-14: Compile-validated via `dotnet build` (no `dotnet test` per task constraints).